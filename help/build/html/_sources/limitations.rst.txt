...
This problem is mitigated, ONLY WHEN YOU USE Step 2: Convert TAB by duplicating the features in :code:`bgt_openbareruimtelabel.gml` so that the GDAL/OGR library can import this layer correctly. However, if you don't have the Python package lxml available to QGIS, this can take a long time.

If you use the tab "Import individual files" or for some reason, the duplication of features fail, the import process will only import the last label position. So, each label object in the gml file will be available in the import, but only one label position defined in the label object is imported. 
